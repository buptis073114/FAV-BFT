...
def merkleVerify(N, val, roothash, branch, index):
    ...
    for br in branch:
        ...
        tindex >>= 1
    # tmphash = base64.b64encode(tmp)
    # str1 = tmphash.decode('UTF-8')
    if tmp != roothash:
...
def reliablebroadcast_change(sid, pid, N, f, leader, input, receive, send):
    """Reliable broadcast
    ...
    if pid == leader:
        ...
        # mmm = uuid.uuid4().bytes + pid.to_bytes(5, 'little')
        # print("mmm is ", mmm)

        stripes = encode(K, N, m)
        mt = merkleTree(stripes)  # full binary tree
        roothash = mt[1]
        # print("type(roothash) is ",type(roothash))
        # print("pid is ", pid)
        for i in range(N):
            branch = getMerkleBranch(i, mt)
            send(i, ('VAL', roothash, branch, stripes[i]))
    ...
    while True:  # main receive loop
        ...
        if msg[0] == 'VAL' and fromLeader is None:
            ...
            if len(ready[roothash]) >= OutputThreshold and echoCounter[roothash] >= K:
                decode_str = decode_output(roothash)
                # print("decode_str is ",decode_str)
                ...
                ret3 = lib.verification_phase(pathencode, evidencefilepathencode, decode_str)
                verification_data = ctypes.string_at(ret3, -1).decode("utf-8")
                # print("verification phase output ", verification_data)
                return decode_str


def reliablebroadcast1(sid, pid, N, f, leader, input, receive, send):
    """Reliable broadcast
    :param int pid: ``0 <= pid < N``
    :param int N:  at least 3
    :param int f: fault tolerance, ``N >= 3f + 1``
